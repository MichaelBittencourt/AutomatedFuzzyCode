...
for i=1:length(fis.Rules)
    ...
    vetorRegras = [vetorRegras, fis.Rules(1,i).Description];
end

regrasMin = [fis.Rules(1,length(fis.Rules)).Description];
dadoTemp = convertStringsToChars(fis.Rules(1,length(fis.Rules)).Description);
dadoTemp = convertCharsToStrings(dadoTemp(1:tam));
regrasTemp = [dadoTemp];
for i=length(fis.Rules):-1:1
...
fprintf('Percentual de redução: %0.2f%%\n\n',100*(1-length(regrasMin)/length(fis.Rules)))
