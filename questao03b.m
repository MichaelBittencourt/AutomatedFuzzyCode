...
rule = [ConjuntosPK1 ConjuntosPK2 ConjuntosPK3 ConjuntosPK4 ConjuntosPK5 ConjuntosSEM(1:length(ConjuntosPK5)) ...
    ConjuntosPOT(1:length(ConjuntosPK5)) (MembPK1.*MembPK2.*MembPK3.*MembPK4.*MembPK5.*MembSEM(1:length(MembPK5))).*MembPOT(1:length(MembPK5)) ones(length(MembPK5),1)];
...
%Autoteste
POTfis = evalfis([PK1 PK2 PK3 PK4 PK5 SEM(1:length(PK1))], fis);  
figure
hold on;
plot (POT);
plot (POTfis);
...
%Com dados de teste 
TESTfis = evalfis([PK1T PK2T PK3T PK4T PK5T SEMT(1:length(PK1T))], fis);      %teste com dado reservado
figure
plot(POTT)
hold on;
plot(TESTfis)
